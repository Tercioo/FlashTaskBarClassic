...
function FlashTaskBar.OnInit (self)
	...
	hooksecurefunc ("StaticPopup_Show", function (token, text_arg1, text_arg2, data, insertedFrame)
		...
		elseif (token == "PARTY_INVITE" or token == "PARTY_INVITE_XREALM" or token == "GROUP_INVITE_CONFIRMATION") then
		...
		elseif (token == "CONFIRM_BATTLEFIELD_ENTRY") then
			FlashTaskBar:DoFlash("group_queue")
		
		elseif (token == "GUILD_INVITE") then
			FlashTaskBar:DoFlash("guild_invite")

		elseif (token == "CAMP") then
			FlashTaskBar:DoFlash("camping")
...
--> build options panel
	...
	local statusBar = CreateFrame ("frame", nil, FlashTaskBar.OptionsFrame1)
	statusBar:SetPoint ("bottomleft", FlashTaskBar.OptionsFrame1, "bottomleft", 5, 5)
	statusBar:SetPoint ("bottomright", FlashTaskBar.OptionsFrame1, "bottomright", -5, 5)
	statusBar:SetHeight (20)
	DF:ApplyStandardBackdrop (statusBar)
	statusBar:SetAlpha (0.97)
	statusBar:SetFrameLevel (FlashTaskBar.OptionsFrame1:GetFrameLevel() + 10)
